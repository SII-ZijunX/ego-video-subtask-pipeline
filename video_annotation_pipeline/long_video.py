...
def prune_stale_prepared_episodes(
    pilot_dir: Path, active_episode_ids: set[str]
) -> dict[str, int]:
    """Remove outputs generated by an earlier prepare run but no longer active.

    Only episode directories carrying this workflow's ``*_long_window`` source
    marker are removed. Unmanaged files under ``dataset/`` are left untouched.
    Qwen JSONL state is filtered to the new active set so resume cannot retain
    annotations for deleted windows.
    """
    dataset_dir = pilot_dir / "dataset"
    analysis_root = (pilot_dir / "analysis_clips").resolve()
    removed_episodes = removed_analysis_clips = removed_qwen_episode_dirs = 0
    for metadata_path in sorted(dataset_dir.glob("*/metadata.json")):
        try:
            metadata = read_json(metadata_path)
        except (OSError, json.JSONDecodeError):
            continue
        episode_id = str(metadata.get("episode_id") or metadata_path.parent.name)
        generated = (
            str(metadata.get("source") or "").endswith("_long_window")
            and metadata.get("label_source") == "qwen_visual_only"
        )
        if not generated or episode_id in active_episode_ids:
            continue
        for camera in metadata.get("cameras") or []:
            media_value = str(camera.get("path") or "").strip()
            if not media_value:
                continue
            media_path = Path(media_value).resolve()
            try:
                media_path.relative_to(analysis_root)
            except ValueError:
                continue
            if media_path.is_file():
                media_path.unlink()
                removed_analysis_clips += 1
            parent = media_path.parent
            if parent != analysis_root and parent.is_dir() and not any(parent.iterdir()):
                parent.rmdir()
        shutil.rmtree(metadata_path.parent)
        removed_episodes += 1

    qwen_dir = pilot_dir / "qwen"
    for state_name in ("annotations.jsonl", "errors.jsonl"):
        state_path = qwen_dir / state_name
        if not state_path.is_file():
            continue
        active_rows = [
            row for row in read_jsonl(state_path)
            if str(row.get("episode_id") or "") in active_episode_ids
        ]
        write_jsonl(state_path, active_rows)
    qwen_episodes = qwen_dir / "episodes"
    if qwen_episodes.is_dir():
        for episode_dir in sorted(path for path in qwen_episodes.iterdir() if path.is_dir()):
            if episode_dir.name not in active_episode_ids:
                shutil.rmtree(episode_dir)
                removed_qwen_episode_dirs += 1
    return {
        "removed_stale_episodes": removed_episodes,
        "removed_stale_analysis_clips": removed_analysis_clips,
        "removed_stale_qwen_episode_dirs": removed_qwen_episode_dirs,
    }
...
def prepare(args: argparse.Namespace) -> None:
    ...
    cleanup = prune_stale_prepared_episodes(
        pilot_dir, {str(row["episode_id"]) for row in windows}
    )
